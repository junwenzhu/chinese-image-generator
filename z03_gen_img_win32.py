...
# 输出路径
output_dir = cfg.path_img_output
if not os.path.exists(output_dir):
    os.makedirs(output_dir)

file_path = os.path.join(output_dir, cfg.txt_gt_name)
# if os.path.exists(file_path):
#     os.remove(file_path)
output = open(file_path, 'a', encoding='utf-8')
...
class SampleGen():
    ...
    def __save_gt(self, line_num, sentence):
        # 写图像文件名和类别序列的对照表
        output.write("{:0>8d}.png\t{}\n".format(line_num, sentence))
        output.flush()  # 重点,在多进程中写文件需要尽快刷新,否则可能会导致数据丢失
